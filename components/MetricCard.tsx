...
export interface MetricCardProps {
  ...
  spark?: {
    ...
    label?: string;
    label2?: string;
    formatter?: string; // "pct" | "cr" | "x" | "ratio"
  };
}

const GRID = { strokeDasharray: "3 3", stroke: "#1A2233" };
const AXIS_STYLE = { fill: "#9FB0C8", fontSize: 10 };
const TOOLTIP_STYLE = {
  contentStyle: {
    background: "#0B1220",
...
export function MetricCard({ title, headline, badge, sentence, sentenceTone, spark }: MetricCardProps) {
  ...
  return (
    <div className="rounded-xl border border-ink-700/60 bg-ink-900/40 p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-widest text-chalk-300/60">{title}</p>
          <p className="num mt-1 text-2xl font-bold text-chalk-50 leading-none">{headline}</p>
        </div>
        {badge && (
          <span className={`shrink-0 mt-0.5 rounded-md border px-2 py-0.5 text-xs font-medium ${badgeBg[badge.tone]}`}>
            {badge.label}
          </span>
        )}
      </div>

      <p className={`text-xs leading-snug ${toneClass[sentenceTone]}`}>{sentence}</p>
          ...
          {spark.type === "comparison" && (
            <ResponsiveContainer width="100%" height={120}>
              <BarChart data={spark.rows} layout="vertical" margin={{ left: 0, right: 8, top: 0, bottom: 0 }}>
                ...
                <Bar dataKey="value" name={spark.label ?? title} fill="#10B981" radius={[0, 3, 3, 0]} maxBarSize={18} />
              </BarChart>
            </ResponsiveContainer>
          )}
            ...
            <ResponsiveContainer width="100%" height={120}>
              <BarChart data={spark.rows.map((r) => ({ label: shortLabel(r.label), value: r.value }))}>
                ...
                <Bar dataKey="value" name={spark.label ?? title}
                  fill="#10B981" radius={[2, 2, 0, 0]} maxBarSize={20} />
              </BarChart>
            </ResponsiveContainer>
          )}
            ...
            <ResponsiveContainer width="100%" height={120}>
              <BarChart data={spark.rows.map((r) => ({ label: shortLabel(r.label), v1: r.value, v2: r.value2 }))}>
                ...
                <YAxis tick={AXIS_STYLE} tickLine={false} axisLine={false} width={42}
                  tickFormatter={(v) => fmt(v, spark.formatter)} />
                <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => fmt(v, spark.formatter)} />
                <Bar dataKey="v1" name={spark.label ?? "Sales"} fill="#10B981" radius={[2, 2, 0, 0]} maxBarSize={14} />
                <Bar dataKey="v2" name={spark.label2 ?? "Net Profit"} fill="#3B82F6" radius={[2, 2, 0, 0]} maxBarSize={14} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
