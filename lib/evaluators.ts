...
export function evaluatePE(raw: CompanyRaw): Evaluation {
  ...
  if (!indPe) return { sentence: `Trades at ${pe.toFixed(1)}x earnings.`, tone: "neutral" };
  const disc = ((indPe - pe) / indPe) * 100;
  if (disc > 20)
    return { sentence: `Trading ${disc.toFixed(0)}% below industry median P/E — relatively cheap vs peers.`, tone: "good" };
  if (disc < -20)
    return { sentence: `Trading ${Math.abs(disc).toFixed(0)}% above industry median P/E — market pricing in high expectations.`, tone: "warn" };
  return { sentence: `P/E is broadly in line with the industry median of ${indPe.toFixed(1)}x.`, tone: "neutral" };
}
...
export function evaluateDividend(raw: CompanyRaw): Evaluation {
  ...
  if (!dy) return { sentence: "No dividend paid currently — earnings are being retained for growth.", tone: "neutral" };
  if (dy >= 3) return { sentence: `Dividend yield of ${dy.toFixed(2)}% — strong income component alongside capital gains.`, tone: "good" };
  if (dy >= 1) return { sentence: `Dividend yield of ${dy.toFixed(2)}% — modest; business retains most earnings.`, tone: "neutral" };
  return { sentence: `Dividend yield of ${dy.toFixed(2)}% — minimal cash returned to shareholders currently.`, tone: "neutral" };
}
...
export function evaluateTrend(cmp: number, raw: CompanyRaw): TrendResult {
  ...
  if (dma50 && dma200) {
    ...
    if      (pct50vs200 >=  5) f3 =  2;   // 50 DMA well above 200 — strong bull structure
    else if (pct50vs200 >=  1) f3 =  1;   // 50 DMA slightly above 200 — early golden cross
    else if (pct50vs200 >= -1) f3 =  0;   // DMAs intertwined — neutral
    else if (pct50vs200 >= -5) f3 = -1;   // 50 DMA slightly below 200 — early death cross
  ...
  if (high52w && low52w && high52w > low52w) {
    ...
    if      (pos >= 0.80) f5 =  2;
    else if (pos >= 0.60) f5 =  1;
    else if (pos >= 0.40) f5 =  0;
    else if (pos >= 0.20) f5 = -1;
    else                  f5 = -2;
  }
  ...
  if (score >= 9) {
    trend = "up"; strength = "strong"; label = "↑↑ Strong Uptrend"; tone = "good";
  } else if (score >= 4) {
    trend = "up"; strength = "mild"; label = "↑ Uptrend"; tone = "good";
  } else if (score >= -3) {
    trend = "sideways"; strength = "neutral"; label = "→ Sideways"; tone = "neutral";
  } else if (score >= -8) {
    trend = "down"; strength = "mild"; label = "↓ Downtrend"; tone = "warn";
  } else {
    trend = "down"; strength = "strong"; label = "↓↓ Strong Downtrend"; tone = "warn";
  }
  ...
  if (stock_1y_cagr !== undefined && stock_1y_cagr !== null) {
    parts.push(`1yr return ${stock_1y_cagr >= 0 ? "+" : ""}${stock_1y_cagr.toFixed(0)}%`);
  }
  ...
  if (dma50 && dma200) {
    if (dma50 > dma200) {
      ...
      else if (trend === "sideways") tail = " — consolidating above long-term support.";
      else                      tail = " — pulling back through moving averages; watch for support.";
    } else {
      if (trend === "down")     tail = " — bearish DMA alignment.";
      else if (trend === "sideways") tail = " — recovery attempt; long-term trend still bearish.";
      else                      tail = " — price testing resistance; longer-term trend still bearish.";
    }
  } else {
  ...
  return { trend, strength, score, label, pctAbove50, pctAbove200, sentence, tone };
}
...
export function evaluateCashConversion(
  ...
  if (pairs.length === 0) {
    return { ratio: null, sentence: "Cash flow data not available for quality assessment.", tone: "neutral" };
  }
  ...
  if (ratio >= 1.0)
    return { ratio, sentence: `CFO averages ${(ratio * 100).toFixed(0)}% of net profit — earnings are fully cash-backed. High quality.`, tone: "good" };
  if (ratio >= 0.7)
    return { ratio, sentence: `CFO covers ${(ratio * 100).toFixed(0)}% of net profit on average — reasonable cash conversion.`, tone: "neutral" };
...
export function evaluateSalesProfit(raw: CompanyRaw): Evaluation {
  ...
  if (!sales && !profit) return { sentence: "5-year growth data not available.", tone: "neutral" };
  if (sales && profit) {
    if (profit > sales * 1.5 && profit >= 15)
      return { sentence: `Profit growing faster than sales (${profit.toFixed(0)}% vs ${sales.toFixed(0)}% 5Y CAGR) — margins are expanding.`, tone: "good" };
    if (sales >= 10 && profit >= 10)
      return { sentence: `Healthy 5Y CAGR: revenue ${sales.toFixed(0)}%, profit ${profit.toFixed(0)}% — a steady compounder.`, tone: "good" };
    if (profit < 0)
      return { sentence: `Profits declining over 5 years despite revenue growth — margin erosion risk.`, tone: "warn" };
    if (sales < 5 && profit < 5)
      return { sentence: `5Y growth is slow: revenue ${sales.toFixed(0)}%, profit ${profit.toFixed(0)}% — not a high-velocity compounder.`, tone: "neutral" };
  }
  if (profit && profit >= 15)
    return { sentence: `5Y profit CAGR of ${profit.toFixed(0)}% — solid long-term earnings growth.`, tone: "good" };
  return { sentence: `5Y growth is moderate — business is stable but not accelerating.`, tone: "neutral" };
}
