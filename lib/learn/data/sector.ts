...
export const sectorMetrics: MetricEntry[] = [
  {
    ...
    category: "sector",
    tagline:
      "The yardstick that turns 'expensive' or 'cheap' into a meaningful answer.",
    inOneSentence:
    ...
    concreteExample: {
      setup:
        "Stock A has a P/E of 22. The IT services sector median P/E is 28.",
      walkthrough:
        "Stock A trades at 22 ÷ 28 = 0.79 - i.e. 21% below sector median.",
      takeaway:
    ...
    formula: {
      plain:
        "Industry P/E = Median P/E across all listed companies in the sector (excluding loss-makers)",
    },
    inputs: [
      {
        name: "Member P/Es",
        meaning: "P/E for every company in the sector with positive earnings",
        source: "Computed",
      },
    ],
    methodology:
    ...
    howToRead: [
      {
        label: "Company P/E < 80% of industry median",
        desc: "Possible undervaluation. Verify with growth, ROCE, and margin trends.",
      },
      {
        label: "Company P/E within ±20% of industry median",
        desc: "Roughly priced in line with peers. Differentiation lies elsewhere.",
      },
      {
        label: "Company P/E > 130% of industry median",
        desc: "Premium pricing. Market is betting on superior fundamentals - check if they're delivering.",
      },
    ],
  ...
  },
  {
    ...
    inputs: [
      {
        name: "Final Score",
        meaning:
          "Raw 10-category score + bonuses − penalties, capped by industry ceiling",
        source: "Scorer",
      },
    ],
    methodology:
    ...
    howToRead: [
      {
        label: "Avoid",
        desc: "Multiple major red flags. Even a price crash usually doesn't change the verdict.",
      },
      {
        label: "Watchlist",
        desc: "One or two structural issues. Worth monitoring for change.",
      },
      {
        label: "Accumulate",
        desc: "Mostly clean. Suitable for incremental building if valuation is reasonable.",
      },
      {
        label: "Invest-grade",
        desc: "Passes every major filter; one or more standout strengths.",
      },
      {
        label: "Exceptional",
        desc: "Top-tier on virtually every dimension. Rare.",
      },
    ],
